...
| **Link summary** | The structured summary the bot posts when a user reacts **📝** (`SUMMARY_REACTION`) to a message containing **any** link — an intro line, then adaptive **Thèse centrale**/**Idée principale**, **Arguments clés**/**Points clés**, and **Questions** (see `buildSummaryFormat` in `prompts.js`). The same format is applied when a link is **@mentioned** to the bot. Opt-in only: the human's 📝 reaction *is* the filter — no host denylist, no auto-summary on post (issues c8dafc0, 71e2200). |
| **Output parsing (`-Q`)** | Extracting the answer from Hermes' **`-Q` programmatic output** (final response only) via `parseHermesOutput` (`prompts.js`) — no `⚕ Hermes` banner, no `Query:` echo. Still the bot's most fragile coupling to the CLI: a change to the `-Q` format can break extraction (issue 9864045). |
