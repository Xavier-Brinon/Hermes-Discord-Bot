...
function runHermes(bin, prompt) {
  ...
  return new Promise((resolve) => {
    execFile(
      ...
      (error, stdout) => {
        if (error) {
          resolve({ ok: false, error: error.message, out: stdout || '' });
          return;
        }
...
async function main() {
  ...
  if (!fs.existsSync(args.fixtures)) {
    console.error(`No fixtures dir: ${args.fixtures}`);
    process.exit(2);
  }
  const files = fs.readdirSync(args.fixtures).filter((f) => f.endsWith('.txt'));
  if (files.length === 0) {
    console.error(`No .txt fixtures in ${args.fixtures}`);
    process.exit(2);
  }
  ...
  for (const file of files) {
    ...
    rows.push({
      file,
      parseable: pct(parseable),
      inRange: pct(inRange),
      clean: pct(clean),
      french: pct(french),
      meanThemes: ok ? (totalThemes / ok).toFixed(1) : '—',
      errors,
    });
  }

  console.log(
    'fixture                 parseable  3-5themes  no-preamble  french  meanThemes  errors'
  );
  for (const r of rows) {
    console.log(
